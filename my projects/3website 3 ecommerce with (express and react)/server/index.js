...
app.post("/login",async(request,res)=>{
        try{
          ...
          const ownerFindByEmail=await owner.findOne({owneremail:email});
          if(ownerFindByEmail!=null){
              console.log(ownerFindByEmail);
              const ownerPassword=ownerFindByEmail.ownerpassword;
              const matchOwnerPass=await bcrypt.compare(userPassword,ownerPassword);
              if(matchOwnerPass){
                console.log("welcome to admin panel");
                let ownerToken;
                if(ownerFindByEmail.tokens.length<1){
                    ownerToken=await ownerFindByEmail.generateOwnerAuthToken();
                    res.send({loginDetails:true,owner:true,loginData:ownerFindByEmail,token:ownerToken})
                    console.log("welecome to admin panel by creating token");
                }
                else{
                    ownerToken=ownerFindByEmail.ownerTokens[0].innerToken;
                    res.send({loginDetails:true,owner:true,loginData:ownerFindByEmail,token:ownerToken})
                    console.log("welecome to admin panel without creating token");
                }
              }
              else{
                console.log("invalid login details");
                res.send({loginDetails:false,owner:false,loginData:null,token:null})
              }
          }
          else{
                ...
                if(studentFindByEmail!=null){
                  ...
                  const matchPassword=await bcrypt.compare(userPassword,databasePassword);
                    if(matchPassword){
                      let token;  
                      if(studentFindByEmail.tokens.length<1){
                          token=await studentFindByEmail.generateAuthToken();
                          res.send({loginDetails:true,owner:false,loginData:studentFindByEmail,token:token,databaseTokens:studentFindByEmail.tokens})
                          console.log("login succeded by creating token");
                      }
                      else{
                          token=studentFindByEmail.tokens[0].innerToken;
                          res.send({loginDetails:true,owner:false,loginData:studentFindByEmail,token:token,databaseTokens:studentFindByEmail.tokens})
                          console.log("login succeded without creating token");
                      }
                    }   
                    else{
                        res.send({loginDetails:false,owner:false,loginData:null,token:null})
...
app.post("/createowner",async(req,res)=>{
    try{
        ...
        const createNewOwner=await newOwner.save();
        res.send();    
    }
    catch(error){
